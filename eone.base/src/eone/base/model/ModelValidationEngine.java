...
//	private VetoableChangeSupport m_changeSupport = new VetoableChangeSupport(this);
	 ...
	 * @return error message or null
	 */
	public String fireFactsValidate (List<Fact> facts, PO po)
	{
		if (facts == null || po == null)
			return null;
		...
		{
			//ad_entitytype.modelvalidationclasses
			String error = fireFactsValidate(facts, po, list);
			if (error != null && error.length() > 0)
				return error;
		}
		...
		FactsEventData eventData = new FactsEventData(facts, po);
	...
	private String fireFactsValidate(List<Fact> facts, PO po,  ArrayList<FactsValidator> list)
	{
		for (int i = 0; i < list.size(); i++)
		{
			...
			try
			{
				...
				{
					String error = validator.factsValidate(facts, po);
					if (error != null && error.length() > 0)
					{
						if (log.isLoggable(Level.FINE))
						{
							log.log(Level.FINE, "po="+po+" validator="+validator);
						}
						return error;
					}
				}
