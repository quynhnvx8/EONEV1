...
public class Group extends org.zkoss.zul.Group {
	...
	private List<eone.webui.component.Row> m_rows = new ArrayList<eone.webui.component.Row>();
	...
	public void add(eone.webui.component.Row row) {
		m_rows.add(row);
	}
	...
	public static class GroupHeader extends Cell implements EventListener<Event>
	{
		...
		private void init()
		{
			setSclass("z-group-header");
			
			Div div = new Div();
			div.setSclass("z-group-header");
			appendChild(div);
			
			span = new Span();
			div.appendChild(span);
			span.addEventListener(Events.ON_CLICK, this);
			span.setSclass("z-group-icon");
			
			lbl = new Label();
			lbl.setSclass("z-group-header");
