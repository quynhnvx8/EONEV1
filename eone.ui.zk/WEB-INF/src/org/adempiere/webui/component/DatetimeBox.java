...
public class DatetimeBox extends Panel {
	...
	public DatetimeBox() {
		//MLanguage language = MLanguage.get(Env.getCtx(), Env.getAD_Language(Env.getCtx()));
		dateBox = new Datebox();
		//dateBox.setFormat(language.getDatePattern());
		dateBox.setCols(10);
		timeBox = new Timebox();
		//timeBox.setFormat(language.getTimePattern());
		timeBox.setCols(5);
	...
	public Date getValue() {
		...
		if (d != null && t != null) {
			...
			cd.set(cd.get(Calendar.YEAR), cd.get(Calendar.MONTH), cd.get(Calendar.DAY_OF_MONTH),
				   ct.get(Calendar.HOUR_OF_DAY), ct.get(Calendar.MINUTE), ct.get(Calendar.SECOND));//, ct.get(Calendar.SECOND)
			d = cd.getTime();
		}
